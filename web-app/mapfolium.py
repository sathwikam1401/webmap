...
data=pd.read_excel("F:\webmap\collegeBtech.xlsx")
data.head()
...
for i in range(0,len(data)):  
    ...
    def popup_html(row):
        i=row
        college_name=data["College name"].iloc[i]
        courses=data["Courses"].iloc[i]
        rating=data["Rating"].iloc[i]
        cutoff=data["Cutoff"].iloc[i]
        place=data["Place"].iloc[i]
    ...
    folium.Marker([data['Latitude'].iloc[i],data['Longitude'].iloc[i]],popup=popup,icon=folium.Icon(color='pink',icon='university',prefix='fa')).add_to(map)
map
map.add_child(fg)
map.save("ts.html")